...
function updatePomodoroBar() {
  ...
  document.getElementById('pomoCountdown').textContent = `${mm}:${ss}`;
  document.getElementById('pomoFill').style.width      = `${(1 - remaining / phaseSecs) * 100}%`;
  document.getElementById('pomoPhase').textContent     = pomoState.phase === 'work' ? '作業中' : '休憩中';
}
...
async function handleStop() {
  ...
  // Show note bar (actual stop happens in confirmStop).
  // Pre-fill with the pending note (task title) if a task drove this run.
  document.getElementById('noteBar').hidden = false;
  const input = document.getElementById('noteInput');
  input.value = timerState.pendingNote || '';
  input.focus();
  input.select();
  updateButtonStates();
}
...
async function confirmStop(skipNote) {
  const note = skipNote ? '' : document.getElementById('noteInput').value.trim();
  document.getElementById('noteBar').hidden = true;
  _processing = true;
  try { await _handleStop(note); } finally { _processing = false; updateButtonStates(); }
}

async function _handleStop(note = '') {
  ...
  if (calendarEnabled) {
    pushEntryToCalendar(entry, clientForCalendar, settings.calendar)
      .then(eventId => {
        if (eventId) showToast(`カレンダーに投入しました🌷`);
      })
      .catch(err => {
...
async function handleSync() {
  ...
  try {
    ...
    await syncToSheet(settings.spreadsheetId, entries, settings.clients, month);
  ...
  window.addEventListener('unload', async () => {
    ...
    if (s.autoSync && s.spreadsheetId) {
      ...
      if (st.status === 'idle') {
        ...
        const entries = await getMonthEntries(month);
        if (entries.length > 0) {
          syncToSheet(s.spreadsheetId, entries, s.clients, month).catch(() => {});
        }
      }
    }
  });
